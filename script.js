...
let produtos = [
  { nome: "Sementes Orgânicas", precoInicial: 5.00, infoSustentavel: "🌱 Cultivo sem agrotóxicos", impactoCO2: "Baixo", icone: "🌱" },
  { nome: "Adubo Natural (kg)", precoInicial: 12.50, infoSustentavel: "🌿 Compostagem orgânica", impactoCO2: "Muito Baixo", icone: "🌿" },
  { nome: "Mudas Nativas", precoInicial: 8.00, infoSustentavel: "🌳 Reflorestamento local", impactoCO2: "Negativo (captura CO₂)", icone: "🌳" }
];
...
function setup() {
  ...
  escala = min(width / 400, height / 800);
  configurarBotoes();
  for (let i = 0; i < 20; i++) {
    bolhas.push({ x: random(width), y: random(height), tamanho: random(10, 50), velocidade: random(0.3, 1.5), tipo: random() > 0.7 ? 'folha' : 'bolha' });
  }
...
function configurarBotoes() {
  ...
  if (isMobile) {
    ...
    let tecX = width/2 - 120;
    let tecY = 280;
    
    let infos = [
      { l: '1', x: 0, y: 0 }, { l: '2', x: 80, y: 0 }, { l: '3', x: 160, y: 0 },
      { l: '4', x: 0, y: 65 }, { l: '5', x: 80, y: 65 }, { l: '6', x: 160, y: 65 },
      { l: '7', x: 0, y: 130 }, { l: '8', x: 80, y: 130 }, { l: '9', x: 160, y: 130 },
      { l: 'C', x: 0, y: 195 }, { l: '0', x: 80, y: 195 }, { l: '←', x: 160, y: 195 }
    ];
    
    for (let b of infos) {
      botoesNum.push({ label: b.l, x: tecX + b.x, y: tecY + b.y, w: 70, h: 55 });
    }
    
    for (let i = 0; i < produtos.length; i++) {
      botoesProduto.push({ id: i, label: produtos[i].icone + " " + produtos[i].nome, x: 15, y: 140 + (i * 50), w: width - 30, h: 45 });
    }
    
    botoesPreco.push({ label: '-', x: width/2 - 100, y: 95, w: 55, h: 45, acao: -0.50 });
    botoesPreco.push({ label: '+', x: width/2 + 45, y: 95, w: 55, h: 45, acao: 0.50 });
    
    botaoAdicionar = { label: '🛒 ADICIONAR AO CARRINHO', x: 15, y: tecY + 270, w: width - 30, h: 50 };
    botaoRemoverUltimo = { label: '↩️ Remover Último', x: 15, y: 0, w: (width - 45)/2, h: 42 };
    botaoLimparCarrinho = { label: '🗑️ Limpar Tudo', x: width/2 + 7, y: 0, w: (width - 45)/2, h: 42 };
    
  } else {
    // Layout DESKTOP
    let infos = [
      { l: '1', x: 60, y: 80 }, { l: '2', x: 140, y: 80 }, { l: '3', x: 220, y: 80 },
      { l: '4', x: 60, y: 145 }, { l: '5', x: 140, y: 145 }, { l: '6', x: 220, y: 145 },
      { l: '7', x: 60, y: 210 }, { l: '8', x: 140, y: 210 }, { l: '9', x: 220, y: 210 },
      { l: 'C', x: 60, y: 275 }, { l: '0', x: 140, y: 275 }, { l: '←', x: 220, y: 275 }
    ];
    
    for (let b of infos) {
      botoesNum.push({ label: b.l, x: b.x, y: b.y, w: 65, h: 50 });
    }
    
    for (let i = 0; i < produtos.length; i++) {
      botoesProduto.push({ id: i, label: produtos[i].icone + " " + produtos[i].nome, x: width/2 - 225 + (i * 225), y: 20, w: 200, h: 55 });
    }
...
function atualizarMensagemSustentavel() {
  let mensagens = ["🌍 Cada escolha sustentável faz a diferença!", "🌱 Agricultura consciente = Futuro verde", "💚 Preservar hoje para colher amanhã", "🌿 Equilíbrio entre produção e natureza", "🍃 Produzir sem destruir é possível!"];
...
function desenharMobile() {
  ...
  textStyle(ITALIC);
  fill('#006666');
  text("Agro forte, futuro sustentável", width/2, 48);
  
  // MENSAGEM SUSTENTÁVEL
  if (millis() - tempoMensagem > 5000) atualizarMensagemSustentavel();
  
  fill(255, 255, 255, 200);
  stroke('#00cc6a');
  strokeWeight(1);
  rect(width/2, 72, width - 30, 22, 10);
  
  noStroke();
  fill('#005500');
  textSize(10);
  textStyle(BOLD);
  text(mensagemSustentavel, width/2, 72);
  
  // PREÇO ATUAL
  fill(255, 255, 255, 180);
  stroke('#00ff87');
  strokeWeight(1.5);
  rect(width/2, 105, width - 60, 35, 10);
  
  noStroke();
  fill('#003366');
  textSize(12);
  text("Preço: " + formatarMoeda(precoPorUnidade), width/2, 105);
  
  // BOTÕES + E -
  for (let bp of botoesPreco) {
    fill(255, 255, 255, 190);
    stroke('#00ff87');
    strokeWeight(1.5);
    rect(bp.x, bp.y, bp.w, bp.h, 10);
    noStroke();
    fill('#004d4d');
    textSize(22);
    textStyle(BOLD);
  ...
  stroke('#00cc6a');
  strokeWeight(1);
  rect(width/2, botoesProduto[2].y + 55, width - 30, 22, 8);
  
  noStroke();
  fill('#003366');
  textSize(10);
  textStyle(ITALIC);
  text(produtos[produtoSelecionado].infoSustentavel + " | CO₂: " + produtos[produtoSelecionado].impactoCO2, width/2, botoesProduto[2].y + 55);
  
  // QUANTIDADE
  let qtdY = botoesProduto[2].y + 90;
  fill(255, 255, 255, 180);
  stroke('#00b4db');
  strokeWeight(1.5);
  rect(width/2, qtdY, 180, 32, 8);
  ...
  stroke('#00b4db');
  strokeWeight(1.5);
  rect(width/2, subY, width - 30, 30, 8);
  ...
  // BOTÃO ADICIONAR
  let ax = botaoAdicionar.x;
  let ay = botaoAdicionar.y;
  
  fill('#00cc6a');
  stroke('#00ff87');
  strokeWeight(2);
  rect(ax, ay, botaoAdicionar.w, botaoAdicionar.h, 15);
  
  noStroke();
  fill(255);
  textSize(15);
  textStyle(BOLD);
  text(botaoAdicionar.label, ax + botaoAdicionar.w/2, ay + botaoAdicionar.h/2);
  ...
  stroke('#00cc6a');
  strokeWeight(1.5);
  rect(width/2, cartY, width - 30, 120, 12);
  ...
  if (carrinho.length > 0) {
    ...
    textSize(13);
    textStyle(BOLD);
    text("🛒 Carrinho (" + carrinho.length + " itens)", width/2, cartY - 45);
    
    textAlign(LEFT);
    for (let i = 0; i < min(carrinho.length, 4); i++) {
      let item = carrinho[i];
      fill('#004d4d');
      textSize(10);
      textStyle(BOLD);
      text((i+1) + ".", 30, cartY - 25 + i * 22);
      text(produtos[encontrarProdutoPorNome(item.nome)].icone + " " + item.nome, 50, cartY - 25 + i * 22);
      fill('#005500');
      text(formatarMoeda(item.subtotal), width - 80, cartY - 25 + i * 22);
    }
    textAlign(CENTER, CENTER);
  } else {
    fill('#999999');
    textSize(13);
    textStyle(ITALIC);
    text("🛒 Carrinho vazio", width/2, cartY - 10);
  }
  ...
  stroke('#00b4db');
  strokeWeight(3);
  rect(width/2, totalY, width - 30, 55, 15);
  
  noStroke();
  fill('#004d4d');
  textSize(13);
  textStyle(BOLD);
  text("💰 TOTAL A PAGAR", width/2, totalY - 12);
  
  textSize(26);
  fill(totalCarrinho > 0 ? '#005500' : '#999999');
  text(formatarMoeda(totalCarrinho), width/2, totalY + 14);
  
  // BOTÕES CARRINHO
  let btnsY = totalY + 50;
  botaoRemoverUltimo.y = btnsY;
  botaoLimparCarrinho.y = btnsY;
  
  // Remover
  fill(carrinho.length > 0 ? '#ffbb33' : '#cccccc');
  stroke(carrinho.length > 0 ? '#ffaa00' : '#999999');
  strokeWeight(1.5);
  rect(botaoRemoverUltimo.x, btnsY, botaoRemoverUltimo.w, botaoRemoverUltimo.h, 10);
  
  noStroke();
  fill(carrinho.length > 0 ? 255 : '#888888');
  textSize(12);
  textStyle(BOLD);
  text(botaoRemoverUltimo.label, botaoRemoverUltimo.x + botaoRemoverUltimo.w/2, btnsY + 21);
  
  // Limpar
  fill(carrinho.length > 0 ? '#ff4444' : '#cccccc');
  stroke(carrinho.length > 0 ? '#ff6666' : '#999999');
  strokeWeight(1.5);
  rect(botaoLimparCarrinho.x, btnsY, botaoLimparCarrinho.w, botaoLimparCarrinho.h, 10);
  
  noStroke();
  fill(carrinho.length > 0 ? 255 : '#888888');
  textSize(12);
  textStyle(BOLD);
  text(botaoLimparCarrinho.label, botaoLimparCarrinho.x + botaoLimparCarrinho.w/2, btnsY + 21);
  ...
  if (carrinho.length > 0) {
    ...
    textSize(11);
    textStyle(BOLD);
    text("🌱 " + calcularImpactoAmbiental(), width/2, btnsY + 55);
  }
}

function desenharDesktop() {
  // PAINEL ESQUERDO
  rectMode(CORNER);
  stroke(255, 255, 255, 180);
  strokeWeight(2);
  ...
  if (mouseX > ax && mouseX < ax + botaoAdicionar.w && mouseY > ay && mouseY < ay + botaoAdicionar.h) {
    fill('#00ff87');
    stroke('#00b4db');
    strokeWeight(3);
  } else {
  ...
  rect(ax, ay, botaoAdicionar.w, botaoAdicionar.h, 20);
  ...
  if (millis() - tempoMensagem > 6000) atualizarMensagemSustentavel();
  ...
  for (let i = 0; i < botoesProduto.length; i++) {
    ...
    rect(bx, by, 300, 45, 12);
    noStroke();
    fill(produtoSelecionado === bp.id ? 255 : '#004d4d');
    textSize(13);
    textStyle(BOLD);
    text(bp.label, bx + 150, by + 22);
  }
  ...
  // BOTÕES CARRINHO
  let rx = botaoRemoverUltimo.x;
  let ry = botaoRemoverUltimo.y;
  ...
  rect(rx, ry, botaoRemoverUltimo.w, botaoRemoverUltimo.h, 12);
  
  noStroke();
  fill(carrinho.length > 0 ? 255 : '#888888');
  textSize(13);
  textStyle(BOLD);
  text(botaoRemoverUltimo.label, rx + botaoRemoverUltimo.w/2, ry + botaoRemoverUltimo.h/2);
  
  let lx = botaoLimparCarrinho.x;
  let ly = botaoLimparCarrinho.y;
  ...
  rect(lx, ly, botaoLimparCarrinho.w, botaoLimparCarrinho.h, 12);
  
  noStroke();
  fill(carrinho.length > 0 ? 255 : '#888888');
  textSize(13);
  textStyle(BOLD);
  text(botaoLimparCarrinho.label, lx + botaoLimparCarrinho.w/2, ly + botaoLimparCarrinho.h/2);
...
function mousePressed() {
  // PRODUTOS
  for (let bp of botoesProduto) {
    let bx = isMobile ? bp.x : (width/2 - 175) + 25;
    let by = isMobile ? bp.y : 110 + (bp.id * 55);
    let bw = isMobile ? bp.w : 300;
    let bh = isMobile ? bp.h : 45;
...
function desenharLayoutDesktop() {
  ...
  if (function normalizarQuantidade(qtd) {
  if (qtd === "" || qtd === "0") return "0";
  return qtd.replace(/^0+/, "") || "0";
}

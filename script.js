...
function setup() {
  ...
  textFont('Arial');
  rectMode(CENTER); // Garante que os retângulos alinhem pelo centro junto com os textos
...
function draw() {
  // Limpa a tela a cada frame mantendo o fundo degradê do CSS visível por trás.
  // Isso substitui o loop antigo que causava o travamento (tela verde/azul estática).
  clear(); 
...
function mousePressed() {
  ...
  for (let i = 0; i < teclas.length; i++) {
    ...
    // Detecção baseada no centro do retângulo (x - largura/2 até x + largura/2)
